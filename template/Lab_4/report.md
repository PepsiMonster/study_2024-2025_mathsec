---
title: "Отчет по лабораторной работе №4: Алгоритмы вычисления наибольшего общего делителя"
...
Цель данной лабораторной работы — изучить различные алгоритмы для вычисления наибольшего общего делителя (НОД) двух или более целых чисел, освоить их реализацию и анализ. В рамках лабораторной работы необходимо реализовать алгоритм Евклида, бинарный алгоритм Евклида, расширенный алгоритм Евклида, а также расширенный бинарный алгоритм Евклида.
...
Алгоритм Евклида используется для нахождения наибольшего общего делителя (НОД) двух целых чисел \( a \) и \( b \), где \( 0 < b \leq a \).

#### Пошаговый алгоритм Евклида

1. **Инициализация**:
   - Задаем начальные значения: \( r_0 \leftarrow a \), \( r_1 \leftarrow b \), \( i \leftarrow 1 \).

2. **Деление с остатком**:
   - Находим остаток от деления \( r_{i-1} \) на \( r_i \). Обозначим этот остаток как \( r_{i+1} \).

3. **Проверка на завершение**:
   - Если \( r_{i+1} = 0 \), то устанавливаем \( d \leftarrow r_i \), где \( d \) — это НОД двух чисел \( a \) и \( b \).
   - Если \( r_{i+1} \neq 0 \), то увеличиваем \( i \) на 1 и повторяем шаг 2.
...
1. **Инициализация множителя**:
   - Задаем \( g \leftarrow 1 \). Этот множитель \( g \) будет использоваться для хранения общего множителя 2, если оба числа \( a \) и \( b \) четные.
...
4. **Основной цикл**:
   - Пока \( u \neq 0 \), выполняем следующие действия:
     - **Шаг 4.1**: Если \( u \) четное, то делим его на 2:
       \[
       u \leftarrow \frac{u}{2}
       \]
     - **Шаг 4.2**: Если \( v \) четное, то делим его на 2:
       \[
       v \leftarrow \frac{v}{2}
       \]
     - **Шаг 4.3**: Если \( u \geq v \), то заменяем \( u \) на \( u - v \); в противном случае заменяем \( v \) на \( v - u \).
...
Расширенный алгоритм Евклида позволяет не только найти НОД двух чисел, но и найти такие целые числа \( x \) и \( y \), что \( ax + by = \text{НОД}(a, b) \). Это уравнение называется линейной комбинацией. Расширенный алгоритм полезен для задач, где требуется найти коэффициенты линейной комбинации, например, в криптографии.
...
2. **Деление с остатком**:
   - Разделяем \( r_{i-1} \) на \( r_i \) с остатком, получая:
     \[
     r_{i-1} = q_i \cdot r_i + r_{i+1}
     \]
   где \( q_i \) — это частное, а \( r_{i+1} \) — остаток.

3. **Проверка на завершение**:
   - Если \( r_{i+1} = 0 \), то устанавливаем:
     \[
     d \leftarrow r_i, \quad x \leftarrow x_i, \quad y \leftarrow y_i
     \]
   - Если \( r_{i+1} \neq 0 \), то обновляем значения коэффициентов:
     \[
...
1. **Инициализация множителя**:
   - Устанавливаем \( g \leftarrow 1 \). Этот множитель будет использоваться для учета общего множителя 2, если оба числа \( a \) и \( b \) четные.
...
4. **Основной цикл**:
   - Пока \( u \neq v \), выполняем следующие действия:
     - **Шаг 4.1**: Если \( u \) четное, то:
       - **4.1.1** Если \( u \) четное, делим его на 2:
         \[
         u \leftarrow \frac{u}{2}
         \]
       - **4.1.2** Если оба числа \( A \) и \( B \) четные, делим их на 2:
         \[
         A \leftarrow \frac{A}{2}, \quad B \leftarrow \frac{B}{2}
         \]
       - В противном случае:
         \[
         A \leftarrow \frac{A + b}{2}, \quad B \leftarrow \frac{B - a}{2}
         \]

     - **Шаг 4.2**: Если \( v \) четное, то:
       - **4.2.1** Если \( v \) четное, делим его на 2:
         \[
         v \leftarrow \frac{v}{2}
         \]
       - **4.2.2** Если оба числа \( C \) и \( D \) четные, делим их на 2:
         \[
         C \leftarrow \frac{C}{2}, \quad D \leftarrow \frac{D}{2}
         \]
       - В противном случае:
         \[
         C \leftarrow \frac{C + b}{2}, \quad D \leftarrow \frac{D - a}{2}
         \]

     - **Шаг 4.3**: Если \( u \geq v \), то:
       \[
       u \leftarrow u - v, \quad A \leftarrow A - C, \quad B \leftarrow B - D
       \]
       В противном случае:
       \[
       v \leftarrow v - u, \quad C \leftarrow C - A, \quad D \leftarrow D - B
       \]
...
В ходе выполнения лабораторной работы были изучены и реализованы различные алгоритмы для нахождения наибольшего общего делителя, включая их расширенные версии. Каждый из алгоритмов имеет свои преимущества и применяется в зависимости от требований к вычислительной эффективности или необходимости в нахождении коэффициентов линейной комбинации. Бинарный алгоритм показал себя как более быстрый для реализации на компьютере, в то время как расширенные алгоритмы обеспечили возможность нахождения дополнительных параметров.
